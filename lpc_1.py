...
CONFIG = {
    # ---- data ----
    "csv_path": "Logistic_LA_15min.csv",
    "data_format": "wide",          # "wide": one column per measurement
                                     ...
                                     # channel column (often literally
                                     # called "C1" in these exports) says
                                     # which measurement that row is --
                                     # e.g. kWh (active) vs kVarh (reactive)
                                     # readings interleaved as rows rather
                                     # than separate columns. See
                                     # `long_format` below and
                                     # `inspect_long_format_channels()`.
    "timestamp_col": "Time",
    "timestamp_unit": "s",          # "s" for Unix epoch seconds, None if
                                     # the column is already parseable text
    ...
    # ---- long-format ("C1" channel-filter) settings ----
    # Only used when data_format == "long". A real meter export commonly
    # stores every reading type (active energy/power, reactive energy/
    # power, ...) as rows of one file, with a column indicating which
    # channel/measurement each row belongs to (the user's file calls this
    # column "C1"). This filters that column to build the active-power
    # (kWh) series, and optionally a reactive-power (kVarh) series too.
    # Run `inspect_long_format_channels(cfg)` once on the real file to see
    # the actual column name and the exact values it holds, then fill
    # these in -- don't guess at the values below, they're placeholders.
    "long_format": {
        "channel_col": "C1",           # column that selects the reading type
        "value_col": "Value",          # column holding the numeric reading
        "kwh_channel_values": ["kWh"], # values in channel_col meaning
                                        # active energy/power -- edit after
    ...
    "power_divisor": 1000.0,        # e.g. 1000.0 to convert W -> kW
...
def load_series_long_format(cfg, return_reactive=False):
    ...
    q_raw = _series_for(lf.get("kvarh_channel_values"))
    return p_raw, q_raw
